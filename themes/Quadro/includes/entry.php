<?php if (get_option('quadro_blog_style') == 'false') { ?>
	<div class="home-post-wrap">
		<div class="single-entry">
			...
			<div class="readmore">
				<a href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf(__('Permanent Link to %s','Quadro'), the_title()) ?>"><?php _e('Read More','Quadro'); ?></a>
			</div>
...
<?php } else { ?>
	<div class="post-wrapper" style="margin-bottom: 20px;">
		...
		<?php if (get_option('quadro_postinfo1') <> '') { ?>
			<div class="articleinfo"><?php _e('Posted','Quadro'); ?> <?php if (in_array('author', get_option('quadro_postinfo1'))) { ?> <?php _e('by','Quadro'); ?> <?php the_author_posts_link(); ?><?php }; ?><?php if (in_array('date', get_option('quadro_postinfo1'))) { ?> <?php _e('on','Quadro'); ?> <?php the_time(get_option('quadro_date_format')) ?><?php }; ?><?php if (in_array('categories', get_option('quadro_postinfo1'))) { ?> <?php _e('in','Quadro'); ?> <?php the_category(', ') ?><?php }; ?><?php if (in_array('comments', get_option('quadro_postinfo1'))) { ?> | <?php comments_popup_link(__('0 comments','Quadro'), __('1 comment','Quadro'), '% '.__('comments','Quadro')); ?><?php }; ?></div>
		<?php }; ?>
