<?php function mytheme_comment($comment, $args, $depth) {
   ...
   <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		   ...
		   <div id="comment-<?php comment_ID(); ?>" class="comment-body clearfix">
		...
		</a><?php edit_comment_link(__('(Edit)','Quadro'),'  ','') ?></div>
					</div>
				</div>
				...
				<div class="comment-wrap">
					...
					<?php if ($comment->comment_approved == '0') : ?>
						<em class="moderation"><?php _e('Your comment is awaiting moderation.','Quadro') ?></em>
						<br />
					<?php endif; ?>
					
					<div class="comment-content"><?php comment_text() ?></div> <!-- end comment-content-->
					<div class="reply-container"><?php comment_reply_link(array_merge( $args, array('reply_text' => __('Reply','Quadro'),'depth' => $depth, 'max_depth' => $args['max_depth']))) ?></div>
